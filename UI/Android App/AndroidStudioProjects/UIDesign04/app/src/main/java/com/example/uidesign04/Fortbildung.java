		...
		if (status.equals("belegt")) {
			...
			for (Fortbildung a : SachbearbeiterEK.gib(name).fortbildungen.keySet()) {
				...
				if (a.getFortbildungTitle().equals(fort)) {
					...
					System.out.println("belegt wurde ausgef�hrt");
					return true;
				}
...
//	}
	...
	public static String druckeFortbildungenStatusReturn(SachbearbeiterEK sachB, String fortbildung) {

		String gebuchteFortbildungen = " Kein Status";

		Fortbildung temp = Fortbildung.gibFortbildung(fortbildung);

		if (sachB.fortbildungen.containsKey(temp)) {
				if (sachB.fortbildungen.get(temp).equals("belegt") || sachB.fortbildungen.get(temp).equals("bestanden")) {

					gebuchteFortbildungen = (sachB.fortbildungen.get(temp));
					return gebuchteFortbildungen;


				} else {
					System.out.println("AusgabeTest " + fortbildung + temp.getFortbildungNamen() + sachB.fortbildungen.size());
					return gebuchteFortbildungen;
